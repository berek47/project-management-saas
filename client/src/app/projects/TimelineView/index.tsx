...
const Timeline = ({
  ...
  const ganttTasks = useMemo(() => {
    return (
      tasks
        ?.filter((task) => {
          const hasTimelineDates = Boolean(task.startDate && task.dueDate);
          const matchesSearch =
            !searchTerm ||
          ...
          return hasTimelineDates && matchesSearch && matchesCompletion;
        })
        .map((task) => ({
  ...
  }, [searchTerm, showCompletedTasks, tasks]);
  ...
  if (error || !tasks) {
    return (
      <div className="px-4 xl:px-6">
        <StatusPanel
          title="We could not load the project timeline"
  ...
  return (
    <div className="px-4 xl:px-6">
      <div className="flex flex-wrap items-center justify-between gap-2 py-5">
        <h1 className="me-2 text-lg font-bold text-slate-900 dark:text-white">
